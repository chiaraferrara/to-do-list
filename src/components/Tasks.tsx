...
import { Task, toggleTaskComplete, utilityGetTasksFromLS, utilityRemoveTaskFromLS } from '../utilities/utilities';
...
function Tasks() {
  ...
  return (
   <>
    { taskList.length === 0  ? (<Wrapper><p> No task available</p></Wrapper>) : ( <Wrapper>
   <ul>
   {tasksList.map((task : any, index : number) => (
    <li ><div key={index}>
        ...
        }>{task.title}</strong>  <Button onClick={() => toggleTaskComplete(task.id)}><img src={doneIcon} alt="check" /></Button>
        </SingleTask>
    </div></li>
